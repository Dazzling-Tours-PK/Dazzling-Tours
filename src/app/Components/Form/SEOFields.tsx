...
const SEOFieldsComponent: React.FC<SEOFieldsProps> = ({
  ...
  const autoGeneratedValues = useMemo(() => {
    ...
    // Only compute meta description if not manually edited
    // Allow regeneration when shortDescription changes (even if meta description has a value)
    if (
      shortDescription &&
      shortDescription.trim() !== "" &&
      !metaDescriptionManuallyEditedRef.current
    ) {
      ...
      if (autoMetaDesc && autoMetaDesc.trim() !== "") {
        ...
        // This allows regeneration when source content (excerpt) changes
        if (!currentMetaDescription || currentMetaDescription.trim() === "") {
          computed.metaDescription = autoMetaDesc;
        } else {
          // If meta description has a value, only regenerate if it matches the auto-generated one
          // This means it was previously auto-generated and should update when source changes
          if (currentMetaDescription === autoMetaDesc) {
            // It's the same, no need to update
          } else {
            // Check if current value looks like it was auto-generated (starts similarly)
            ...
            if (
              currentStart === autoStart ||
              currentMetaDescription.length < 20
            ) {
              computed.metaDescription = autoMetaDesc;
            }
          }
  ...
  useEffect(() => {
    ...
    let updated = false;
    // Always preserve all existing fields, including focusKeyword and ogImage
    ...
    // Update meta description if computed and different (only if not manually edited)
    if (
      !metaDescriptionManuallyEditedRef.current &&
    ...
    currentMetaTitle,
    currentMetaDescription,
    values.focusKeyword, // Include focusKeyword to preserve it
    values.ogImage, // Include ogImage to preserve it
  ]);

  const handleFieldChange = (field: keyof SEOFieldsType, value: string) => {
    // Always ensure all SEO fields are included when updating
    onChange({
      metaTitle: values.metaTitle ?? "",
      metaDescription: values.metaDescription ?? "",
      slug: values.slug ?? "",
    ...
    <div className={`form-section ${className}`}>
      ...
      <div className="form-group">
        <Textarea
          ...
          onChange={(value) => {
            // Only mark as manually edited if user makes a significant change
            // (more than just a single character or clearing the field)
            const currentValue = values.metaDescription || "";
            const isSignificantChange =
              value.length > 1 || // More than one character
              (currentValue.length > 10 &&
                value.length < currentValue.length - 5); // Significant deletion
